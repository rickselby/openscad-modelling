...
function sumVector(v, i) = (i < 0 ? 0 : (i == 0 ? v[i] : v[i] + sumVector(v, i - 1)));
function makeVectorTotal(v, i = 0) = (i == len(v) ? [] : concat(sumVector(v, i), makeVectorTotal(v, i + 1)));
function subVector(v, i = 1) = (i == len(v) ? [] : concat(v[i], subVector(v, i + 1)));
...
function makeVectorLengths(v, i = 0) = (i == len(v) ? [] : concat(v[i] - prevVal(v, i), makeVectorLengths(v, i + 1)));
function prevVal(v, i) = (i <= 0 ? 0 : v[i - 1]);
